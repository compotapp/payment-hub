...
-- Применяем триггер к таблице accounts
CREATE TRIGGER trigger_accounts_updated_at
    BEFORE UPDATE ON accounts
    FOR EACH ROW
    EXECUTE FUNCTION update_updated_at_column();

COMMENT ON TRIGGER trigger_accounts_updated_at ON accounts IS 'Автоматически обновляет updated_at при изменении записи';